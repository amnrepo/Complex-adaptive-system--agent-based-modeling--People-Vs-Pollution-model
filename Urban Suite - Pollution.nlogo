...
;  set-default-shape carss "car"

  ask patches [
    set pollution 0
    set is-power-plant 0
...
;    ]
;
    if strategy = "advance" or strategy = "advance2"[
;;       set ycor random 31
;;
;;      ;set xcor
;;        set xcor random 0
;;       set xcor random -35 35
;
;
;;     set-plot-x-range random min-pxcor max-pxcor
;     ;set-plot-y-range 0 31
;      ;setxy (-0.8 * max-pxcor) (0.8 * max-pycor)
    setxy random-float min-pycor random-float max-pycor
;
    ]
    if strategy = "delay" [
    setxy random-pxcor random-pycor
...
;    set is-power-plant power-plants
;  ]
;end
...
to create-power-plants
  ask patches [
    if strategy = "delay" or strategy = "advance"
    [

;      if power-plants = 1
;      [
;        if (distancexy (-0.8 * max-pxcor) (0.8 * max-pycor)) < 0.5
;        [ set pp-number 1 ]
;      ]
      if power-plants = 8
      [
        if (distancexy (0.6 * max-pxcor) (-0.8 * max-pycor)) < 0.3
        [ set pp-number 1 ]

        if (distancexy (0.0 * max-pxcor) (-0.8 * max-pycor)) < 0.3
        [ set pp-number 2 ]

        if (distancexy (-0.2 * max-pxcor) (-0.8 * max-pycor)) < 0.3
        [ set pp-number 3 ]

        if (distancexy (0.8 * max-pxcor) (-0.8 * max-pycor)) < 0.3
        [ set pp-number 4 ]

        if (distancexy (0.4 * max-pxcor) (-0.8 * max-pycor)) < 0.3
        [ set pp-number 5 ]
    ...
    [
      ...
      [
        ...
        if (distancexy (-0.8 * max-pxcor) (0.0 * max-pycor)) < 0.3
        [ set pp-number 3 ]
...
;  rt random-float min-pycor

 if strategy = "advance" or strategy = "advance2"[
;
  rt random-float max-pxcor
  lt random-float min-pxcor
;
  ]
;;  lt random-float max-pycor
;
   if strategy = "delay" [
  rt random-float 50
  lt random-float 50
...
to reproduce  ;; person procedure
  if health > 4 and random-float 1 < birth-rate  [
    hatch-people 1 [
      set health 5
      if people-pollution[
        set pollution pollution + (polluting-rate) * 1.85]
...
to count-nbrs             ;; mark only the cells with 'just enough' neighbors
  let n count (people-on neighbors)
  if (n = 8) [die];; 'live' next generation
end
